...
/**
 * Simple MMIO Bundle for VGA peripheral
 * Compatible with CoralNPU's memory interface
 */
class VgaMMIOBundle extends Bundle {
 ...
 * Adapted for CoralNPU Standalone
 *
 * Memory map (Base: 0x50000):
...
class VGA extends Module {
  val io = IO(new Bundle {
    val mmio        = new VgaMMIOBundle // MMIO interface (CPU clock domain)
  ...
  withClock(sysClk) {
    ...
    val addr             = io.mmio.address & 0xff.U // VGA registers at 0x00-0x5F
    ...
    val palette_idx      = ((addr - 0x20.U) >> 2)(3, 0) // Only 4 bits needed for 16 entries
    ...
    // MMIO Read
    io.mmio.read_data := MuxLookup(addr, 0.U)(
      Seq(
    ...
    when(io.mmio.write_enable) {
      when(addr_ctrl) {
        ...
        val requested_frame = io.mmio.write_data(7, 4)
        val display_enable  = io.mmio.write_data(0)
        when(requested_frame < 12.U) {
          ctrlReg := io.mmio.write_data
        }.otherwise {
        ...
        intrStatusReg := intrStatusReg & ~io.mmio.write_data
      }.elsewhen(addr_upload_addr) {
        uploadAddrReg := io.mmio.write_data
        ...
        fb_write_data := io.mmio.write_data
        ...
        paletteReg(palette_idx) := io.mmio.write_data(5, 0)
      }
    }
