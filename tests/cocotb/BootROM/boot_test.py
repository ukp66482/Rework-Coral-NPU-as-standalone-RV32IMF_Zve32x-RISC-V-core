"""
CocoTB Boot Test for CoralNPU Standalone Boot

This test verifies the BootROM -> FSBL -> Application boot flow.
It simulates external Flash by responding to AXI Master read requests.

Memory Map (Default):
  ITCM:    0x00000000 (8KB)
  DTCM:    0x00010000 (32KB)
  CSR:     0x00030000 (4KB)
  BootROM: 0x00040000 (8KB)
...
# Memory configuration for Default
FLASH_BASE = 0x20000000
MAILBOX_BASE = 0x00030000
...
@cocotb.test()
async def standalone_boot_test(dut):
    """Test standalone boot: Load app to ITCM via FSBL simulation."""
    
    # 1. Initialize Interface
    #    Use ext_mem_base_addr=0x20000 to capture Mailbox writes at 0x30000
    #    ITCM at 0x0 goes through real AXI transactions
    core_mini_axi = CoreMiniAxiInterface(dut, ext_mem_base_addr=0x20000, ext_mem_size=0x20000)
    ...
    # 3. Load test application binary
    r = runfiles.Create()
    app_path = r.Rlocation("coralnpu_hw/tests/standalone_boot/app.bin")
    
    with open(app_path, "rb") as f:
        app_binary = f.read()
    
    # 4. WORKAROUND: Load app directly to ITCM
    #    (Same as C++ boot_test, because Verilator doesn't support $readmemh in bind)
    ...
    mailbox_offset = MAILBOX_BASE - 0x20000
    ...
    # 7. Read mailbox from internal memory buffer and verify
